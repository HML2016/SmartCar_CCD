...
/*Is_p()
{
  int i;
  for(i=15;i>10;i--){
   if(Previous_Error[i]<Previous_Error[i-1]) {
    if(Previous_Error[i-1]<Previous_Error[i-2])
       if(Previous_Error[i-2]<Previous_Error[i-3])
       if(Previous_Error[i]>-10&&Previous_Error[i-3]<0) {
        p_flag=1; 
        break;
       }
     }
