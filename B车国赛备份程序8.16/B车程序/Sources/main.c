...
{   
    ...
    L_speed=(int)steer_control_P;
    R_speed=100;
    S=Current_speed*100;
    OLED_PutString(0, 0, "ROADTYPE:");   //显示字符
    OLED_PrintValueI(64,0, RoadType);
    OLED_PutString(0, 1, "R_speed:");
    OLED_PrintValueI(64,1, steer_control_P);
    OLED_PutString(0, 2, "Middle_Err:");
    OLED_PrintValueI(64,2, Middle_Err);
    OLED_PutString(0, 3, "Speed:");
    OLED_PrintValueI(64,3, t);
}
     ...
     while(PORTE_PE0==1) //S1开始按键 
    {
      ...
      while(PORTE_PE2==1){  
        switch(PTH){
          ...
          case 0xfd:setspeed=75;Fuzzy_Kp=0.23;Fuzzy_Kd=2;TURNP=4;TURND=25;Li=4;Ri=4;superspeed=90;break;
          ...
          case 0xef:setspeed=85;Fuzzy_Kp=0.23;Fuzzy_Kd=2;TURNP=4;TURND=25;Li=4;Ri=4;superspeed=95;break;  //5
...
interrupt VectorNumber_Vpit0 void PIT_ISR(void) 
{
    ...
    switch(part)
  {
    ...
    case 3: 
            ...
            M_PACNT=PACNT;
            PACNT=0;
            Current_speed=(int)((M_PACNT)*2);
            Current_speed1=(int)(PORTB*1.65);
            part = 0;   //循环计数归零
            break;         
    default:
           break;
