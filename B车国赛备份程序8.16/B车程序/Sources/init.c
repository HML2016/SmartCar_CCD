...
/***************总线频率的设定****************************/
void SetBusCLK_80M(void)//设置总线时钟频率是80M
{      
  ...
  SYNR =0xc0 | 0x09;  // VCO_clock=2*osc_clock*(1+SYNR)/(1+REFDV)=160 MHz; 时钟合成器寄存器  SYNR寄存器来控制锁相环的倍频系数
                        // VCOFRQ[1:0]=1:1,代表VCO_clock在80~160 MHz之间    并确定VCOCLK的频率范围
                            
    /* REFDV = (REFFRQ1 | REFFRQ1 | REFDV5 | REFDV4 | //
                 REFDV3 | REFDV2 | REFDV1 | REFDV0 | //
                )   */                                              
  REFDV=0x80 | 0x01;// VCO_clock=2*osc_clock*(1+SYNR)/(1+REFDV)=160 MHz;
...
void PWM_Init(void)
{
    ...
    PWMCTL_CON01 = 1;					 // 16位级联PWM01 舵机通道
